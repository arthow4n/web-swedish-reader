...
export const fetchAIExplanation = async (word: string, context: string): Promise<string> => {
  ...
  // Ensure the prompt supports multiple languages based on the current setting
  const { getCurrentSourceLanguage } = await import("./dictionaryDatabase");
...
<div class="ai-explanation-container" style="display: none; padding: 10px; background: #f5f5f5; border-radius: 4px; font-size: 0.9em;">
  <div class="ai-explanation-meta" style="font-size: 0.8em; color: #666; margin-bottom: 8px;"></div>
...
const aiExplanationMeta = document.querySelector(".ai-explanation-meta");
...
aiExplainButton?.addEventListener("click", async () => {
  ...
  try {
    const result = await fetchAIExplanation(queryInput.value, currentContext);
    ...
    const timeInSeconds = (result.timeTakenMs / 1000).toFixed(2);
    aiExplanationMeta!.textContent = `Generated by ${result.modelUsed} in ${timeInSeconds}s`;
  } catch (err: any) {
...
### 6. Open Suggestions for Future Enhancements

While the implementation above covers the core PRD, consider these enhancements for a more polished user experience:

- **Response Streaming:** Instead of waiting for the full generation to complete, implement Server-Sent Events (SSE) streaming by setting `stream: true` in the OpenRouter payload. This allows the user to read the explanation as it is generated, significantly reducing perceived latency.
- **Local Response Caching:** Explanations for specific word/context pairs can be cached in `localStorage` or `IndexedDB`. If the user clicks the same word in the same sentence again, the app could retrieve the cached explanation instantly, saving API credits and time.
- **Customizable Prompts in Settings:** Allow power users to customize the `systemPrompt` directly in the Settings modal, enabling them to request different formatting or focus areas (e.g., more grammar focus vs. simple translation).
- **Auto-fetch Preference:** Add a setting to automatically trigger the "AI Explain" fetch whenever a word is clicked, bypassing the need to manually click the "AI" button, for users who prefer AI explanations over static dictionaries.
