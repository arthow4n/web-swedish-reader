import { wordClassName, toWordSpans, uniq, isElementVisible, getContextForElement } from "./utils";
import { fetchAIExplanation } from "./llm";
import { openRouterApiKeySetting } from "./settings";
import { currentSelectedWordElementInArticle } from "./index";
import {
  queryCompounds,
...
if (!(aiExplainButton instanceof HTMLButtonElement)) {
  throw new Error("AI explain button not found");
}
if (!(aiExplanationContainer instanceof HTMLElement)) {
  throw new Error("AI explanation container not found");
}
if (!(aiExplanationContent instanceof HTMLElement)) {
  throw new Error("AI explanation content not found");
}
if (!(aiExplanationMeta instanceof HTMLElement)) {
  throw new Error("AI explanation meta not found");
}
...
aiExplainButton.addEventListener("click", async () => {
  ...
  if (!currentSelectedWordElementInArticle) {
    alert("Please select a word in the text first.");
    return;
  }
  ...
  try {
    const context = getContextForElement(currentSelectedWordElementInArticle);
    const result = await fetchAIExplanation(queryInput.value, context);
